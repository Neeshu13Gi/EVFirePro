...
/* ------------------------------------------------------------------ */
/*  Direct paths – NO IMPORTS = full-resolution assets                */
/* ------------------------------------------------------------------ */
const logo           = '/assets/images/EVlogo.png';
const suppressionImg = '/assets/images/ev fire pro.png';
const batteryFireImg = '/assets/images/EV fire.png';
const featuresImg    = '/assets/images/EV Features.png';
const understandingImg = '/assets/images/understanding.png';
const backIcon       = '/assets/images/back.png';
const titleButton    = '/assets/images/title_Button.png';
const thermalVideo   = '/assets/videos/thermalRunaway.mp4';
const OccupantWarning= '/assets/videos/Occupant warning system.mp4';
...
const understandingContent = [
  { title: 'What is thermal runaway?', text: 'A self-sustaining chemical reaction in a lithium-ion cell — often triggered by battery damage, overcharging, or a fault — that overheats and ignites surrounding cells.', button: 'Thermal Runaway', video: thermalVideo },
  { title: 'What are toxic gases in EV fires?', text: 'Burning batteries release: • Hydrogen fluoride (HF) • Carbon monoxide (CO) • Carbon dioxide (CO2) • Methane (CH4) • Hydrogen (H2) and other toxic vapours creating life-threatening risks for responders.', button: 'Toxic Gases', video: thermalVideo },
  { title: 'Understanding Deflagration', text: 'Deflagration occurs when battery cells rapidly release stored energy, causing explosive combustion — requiring specialized suppression techniques.', button: 'Deflagration', video: thermalVideo },
  { title: 'Intense Jet-like Flames', text: 'Fires can eject jet flames over 1,000°C sideways – igniting vehicles and damaging nearby structures.', button: 'Jet-like Flames', video: thermalVideo },
  { title: 'EV Fires in Enclosed Spaces', text: 'Fires release heat and toxic smoke that can weaken structures and force evacuation.', button: 'EV Fires in Car Parks', video: thermalVideo },
  { title: 'About Electric Vehicles', text: 'Modern EVs use high-capacity lithium-ion batteries. Understanding fire risks and response is vital for safety personnel.', button: 'About EV Vehicles', video: thermalVideo },
];
      ...
      <Container>
        <Frame>
          <LogoContainer>
            <Logo src={logo} alt="EV Fire Protection" />
            <button
              style={{ position: 'absolute', top: 21, right: 18, padding: '4px 6px', border: 'none', background: 'transparent', cursor: 'pointer', zIndex: 20 }}
            ...
            <div className={styles.featuresFrame}>
              ...
              <div className={styles.featuresTop}>
                <div className={styles.videoBoxFeatures}>
                  <video ref={featuresVideoRef} src={current.video} autoPlay loop muted playsInline controls />
                </div>
                ...
                <div className={styles.buttonListFeatures}>
                  {featuresContent.map((item, i) => (
                    <button
                      key={item.button}
                      className={`${styles.titleButtonFeatures} ${i === featuresIdx ? styles.active : ''}`}
                      style={{ background: `url(${titleButton}) no-repeat center center`, backgroundSize: '100% 100%' }}
